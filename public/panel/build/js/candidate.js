$("#ExperienceTable, #EducationTable, #DocumentTable").DataTable({
    "responsive": false,
    "lengthChange": true,
    "autoWidth": true
});
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                if(xhr.status == 422){
                    var res = JSON.parse(xhr.responseText);
                    var text = '';
                    if('education_scu' in res.errors){$("#education_scu").focus();}
                    if('education_main_subject' in res.errors){$("#education_main_subject").focus();}
